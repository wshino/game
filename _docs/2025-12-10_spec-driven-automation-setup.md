...
PRが作成されると自動でレビューが実行され、「🚨 必須修正」が指摘された場合は Claude Assistant が自動で修正を行う。

### トリガー条件

1. **自動レビュー**: PR作成時 (`direct_prompt` 使用)
2. **自動修正**: PRコメントに「🚨 必須修正」が含まれる場合
   - ただし「修正が必要な問題はありません」を含む場合は除外 (LGTMの場合)

### 対応内容

- 「🚨 必須修正」: **自動対応** (Claude Assistant が修正)
- 「🔧 改善提案」: **対応不要** (任意)
